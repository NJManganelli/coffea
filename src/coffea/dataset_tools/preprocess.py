...
def _preprocess_pydantic(
    ...
    # Short-circuit an empty group so we don't import dask or issue an (empty) dask.compute.
    # preprocess() filters a fileset into per-format groups, so for a single-format fileset
    # one of those groups is empty and would otherwise pay for a wasted compute.
